...
const BuyCredit = () => {
    ...
    const options = {
      ...
      handler: async(response)=>{
        try {
          ...
          const {data}=await axios.post(backendURL+'/api/user/verify_razor',response,{headers: {Authorization: `Bearer ${token}`}})
  ...
  const paymentRazorpay=async (planId) =>{
    ...
    try {
      ...
      const {data}=await axios.post(backendURL+'/api/user/pay_razor',{planId},{headers: {Authorization: `Bearer ${token}`}
      })
  ...
  return (
    ...
    className='m-h-[80vh] text-center pt-14 m-10' >
